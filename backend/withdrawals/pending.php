...
if(isset($userId)){
	...
	if($count == 1){
        ...
        $stmt = $con->prepare("SELECT SUM(`amount`) AS `totalPending` FROM `transactions` WHERE `username` = ? AND `type` = 'depositWithdrawal' AND `status` = 'pending'");
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            $response = array('status' => 'success', 'message' => 'Pending withdrawal Transaction History', 'pending' => $pending);
            echo json_encode($response);
        }  else {
            $response = array('status' => 'error', 'message' => 'No withdrawal Transaction Found', 'pending' => '0');
            echo json_encode($response);
        }
