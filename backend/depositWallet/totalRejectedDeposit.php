...
if(isset($userId)){
	...
	if($count == 1){
        ...
        $stmt = $con->prepare("SELECT SUM(`amount`) AS `totalRejected` FROM `transactions` WHERE `username` = ? AND `type` = 'depositWalletCredit' AND `status` = 'rejected'");
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            $response = array('status' => 'success', 'message' => 'rejected Deposit Transaction History', 'rejected' => $rejected);
            echo json_encode($response);
        }  else {
            $response = array('status' => 'error', 'message' => 'No Deposit Transaction Found', 'rejected' => '0');
            echo json_encode($response);
        }
