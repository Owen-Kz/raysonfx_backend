...
if(isset($userId)){
	...
	if($count == 1){
        ...
        $stmt = $con->prepare("SELECT SUM(`amount`) AS `totalPending` FROM `transactions` WHERE `username` = ? AND `type` = 'depositWalletCredit' AND `status` = 'pending'");
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            $response = array('status' => 'success', 'message' => 'Pending Deposit Transaction History', 'pending' => $pending);
            echo json_encode($response);
        }
        else {
            $response = array('status' => 'error', 'message' => 'No Deposit Transaction Found', 'pending' => '0');
            echo json_encode($response);
        }
